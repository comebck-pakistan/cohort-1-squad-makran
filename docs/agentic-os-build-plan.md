...
- Pre-meeting briefing → when a meeting is confirmed (as built: the `meeting/confirmed` event fired by `calendar-sync` for a known-client match, or by the user confirming a suggestion), trigger an Inngest function that does `step.sleepUntil(meetingStart - 15min)` then sends the briefing. The calendar half is Solvo's own Google OAuth + poll, not Recall's Calendar Integration, see features.md 4.7.
...
M4  Meetings pipeline (Skribby + Inngest + ticketization)                  : DONE (both paths verified: manual-paste local, Skribby bot-join against a real live Google Meet call) [was M6]
M5  Agent Runtime (Inngest steps + GitHub API + PR flow)                   : code-complete, GitHub OAuth connect + repo picker verified live 2026-08-04, agent-run loop itself (plan->PR->merge) still not E2E tested [was M7]
...
**Status: DONE, both paths fully verified. Vendor swapped Recall.ai → Skribby on 2026-08-03 (Recall.ai's signup form rejects personal email domains, user has no business email; Skribby has no such wall). Bot-join path rewritten against Skribby's real, live-fetched API docs (`docs.skribby.io`), then verified for real the same day: user started a live Google Meet call, spoke a scripted 3-item update, and gave the join link. A real bot was created via the real `SKRIBBY_API_KEY` and it actually joined that live call (`booting → joining → recording`, confirmed by polling `GET /bot/{id}`, not assumed). The bot was stopped via `POST /bot/{id}/stop` (same effect as the meeting ending) and reached `finished` (`recording → leaving → processing → transcribing → finished`), returning a real Whisper transcript (minor ASR typos, all 3 spoken items intact) and a real `recording_url`. That transcript was run through the actual `ticketizeTranscript()` (real `gpt-5-nano` call, not a stub) and produced 4 correct, genuinely actionable draft tickets, one per real issue plus a coupon-specific split. The webhook route's HMAC-SHA256 verification was separately proven both ways (`200` on a correctly-signed synthetic payload, `401` on a bad one) since Skribby's cloud can't deliver a live webhook to `localhost:3000` (no tunnel); the DB-write/Inngest-trigger side of that route already matches the pattern proven working in M7's webhook-adjacent Inngest triggers.**

**Scope, as built**
- pgvector-adjacent schema addition: `meetings.transcript_text` column (nullable), discovered mid-build, see deviations below.
- `lib/skribby.ts`: create-bot (`POST /bot`, auto-detects `service` from the meeting URL: zoom/teams/gmeet), get-transcript + get-recording-url (both read `GET /bot/{id}`, Skribby bundles everything into one bot object once `finished`) against Skribby's real documented REST API. `app/api/webhooks/skribby/route.ts`: HMAC-SHA256 signature verification (`X-Skribby-Signature`/`X-Skribby-Timestamp`, 5-min replay tolerance, Skribby's own scheme, not Svix), fast-ack, DB status update + `meeting/ready-for-processing` Inngest event on `status_update` → `finished`.
...
- Recall.ai has no local/mock substitute and its signup wall (business email required) blocked any real account; user explicitly deferred it on 2026-08-02 and moved to M8/M9. On 2026-08-03, after M9 was done, user got a real Skribby API key (no signup wall) and asked to swap vendors. Rewrote `lib/skribby.ts`/`app/api/webhooks/skribby/route.ts` against Skribby's real docs (fetched live via browser, not guessed): renamed `meetings.recall_bot_id` → `skribby_bot_id` and the `'bot_recall'` source enum value → `'bot_skribby'` (migration `20260803100000_rename_recall_to_skribby.sql`), applied locally, types regenerated. Removed the now-unused `svix` dependency (Skribby signs webhooks with its own HMAC-SHA256 scheme, not Svix). The manual-paste path remains what's actually been verified end-to-end; the bot-join path is unverified against a live call this turn, only against the real API's documented shape.

**Exit criteria, verified (manual-paste path):** a real test meeting (pasted transcript) flows `processing → ready`, produces correct, genuinely actionable draft tickets (verified against a scripted test transcript), and "Confirm & create tickets" correctly promotes them into real `backlog` tickets. A known-client meeting (seeded test client/contact, since real Clients data doesn't exist until M9) correctly fires the pre-meeting briefing email at the 15-minute mark, with real cached verdict/price-band/proposal data, to the freelancer's own inbox.

**Exit criteria, verified (Skribby bot-join path, 2026-08-03):** a real bot joined a real live Google Meet call end to end (`booting → joining → recording → leaving → processing → transcribing → finished`, polled directly against Skribby's API, not simulated), returned a real transcript and recording URL, and that real transcript correctly ticketized via the actual `ticketizeTranscript()` into 4 accurate draft tickets. Not exercised this turn: the live webhook *delivery* itself (Skribby cloud → `localhost:3000` isn't reachable without a tunnel) and the full click-path through `scheduleBotMeeting`/the Meetings UI (the direct-API verification above exercises the same `lib/skribby.ts` functions those call). The webhook route's own signature-verification and status-mapping logic was proven separately with signed synthetic payloads.

---

### 8. M5 — Agent Runtime *(was M7)*

**Status: code-complete. `tsc`/`eslint`/`npm run build` all clean. GitHub auth swapped from a shared `GITHUB_TOKEN` PAT to real per-user OAuth on 2026-08-04 (see deviation below) and verified live end-to-end: real "Connect with GitHub" redirect, real code-for-token exchange, real encrypted storage, real `GET /user/repos` populating the repo picker, real repo connected via `verifyRepoAccess` with the stored token. The agent-run loop itself (plan→approve→execute→PR→review→merge, plus the 3-attempt retry-cap case) is still not E2E tested, still needs a real ticket assigned against the connected test repo.**
...
- `lib/github.ts`: repo verification, branch creation, per-file commits via the Contents API (author/committer hard-set to `Solvo Agent <agent@solvo.dev>` on every call, regardless of the token's identity), PR open/merge, and a check-runs poller that rolls multiple check-runs up into one pending/success/failure verdict.
...
**Discovered during build, deviates from the original spec:**
- Handoff's locked stack said "OAuth (not GitHub App)" for GitHub. Originally deviated to a shared `GITHUB_TOKEN` PAT (2026-08-02) because Supabase Auth's own "Sign in with GitHub" doesn't request `repo` scope and doesn't durably persist `provider_token` past the callback. **Resolved 2026-08-04**: built a second, app-owned GitHub OAuth App (separate client id/secret from the sign-in one, since classic OAuth Apps only support one callback URL each) with its own Authorization Code flow (`app/api/github/oauth/start`, `app/api/github/oauth/callback`), `repo workflow` scope. The callback exchanges the code, reads the authenticated login, and stores the token AES-256-GCM-encrypted (`lib/crypto.ts`, key from `INTEGRATION_TOKEN_ENCRYPTION_KEY`) in a new `integrations.access_token` column. `lib/github.ts`'s functions all take the token as an explicit argument now, no env read. Settings → Integrations now has a real "Connect with GitHub" button and a real repo picker (`GET /user/repos`) instead of a free-text `owner/repo` box, both verified live against a real GitHub account.
...
**Exit criteria, partially verified:** GitHub OAuth connect + repo picker + repo connect confirmed live on 2026-08-04 against a real GitHub account (`@M-IBRAHIM-PRO`, repo `M-IBRAHIM-PRO/co-pilot_testing`). Still not run: a real ticket assigned against that repo generating a plan, gating at plan approval, executing (branch + commit with `Solvo Agent <agent@solvo.dev>` as both author and committer), gating again at PR review, and merging on approval; and a second ticket against a repo with a workflow that always fails on `pull_request` looping three times to `needs_human` (`agent_runs` showing 3 attempt rows, `attempt_count` at 3). Code is otherwise believed correct pending that real-world run.
...
- No app icon graphic exists anywhere in the design assets (only the "Solvo" text wordmark), Plasmo's build fails without one. Generated a simple placeholder 512×512 PNG (`--signal` colored square with a white "A") via ImageMagick at `extension/assets/icon.png`; a real icon design is a fair thing to revisit later but wasn't blocking for a mock-data milestone.
...
- **Auth bridge, not in the original scope bullets:** the extension is a separate origin with no session cookie and can't safely embed `OPENAI_API_KEY`. Added a small `extension_tokens` table (owner_id, token) and a Settings → Integrations "Browser extension" card (generate/show-once Bearer token, same trust level as the GitHub OAuth token), pasted into the extension's sign-in screen, stored in `chrome.storage.local`. `proxy.ts`'s auth gate needed the same `/api/extension/` exclusion M4 already needed for `/api/inngest` and `/api/webhooks/`, same bug class, caught before it shipped this time.
