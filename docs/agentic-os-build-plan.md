...
**Exit criteria, partially verified:** GitHub OAuth connect + repo picker + repo connect confirmed live on 2026-08-04 against a real GitHub account (`@M-IBRAHIM-PRO`, repo `M-IBRAHIM-PRO/co-pilot_testing`). Still not run: a real ticket assigned against that repo generating a plan, gating at plan approval, executing (branch + commit with `Agentic OS Agent <agent@agentcos.dev>` as both author and committer), gating again at PR review, and merging on approval; and a second ticket against a repo with a workflow that always fails on `pull_request` looping three times to `needs_human` (`agent_runs` showing 3 attempt rows, `attempt_count` at 3). Code is otherwise believed correct pending that real-world run.
